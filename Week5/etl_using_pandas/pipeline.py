...
def setup_logging(log_file=None):
    ...
    filename = log_file or os.getenv("ETL_LOG_FILE") or "pipeline.log"
    file_handler = logging.FileHandler(LOG_DIR / filename, mode="w")
...
def parse_args():
    ...
    parser.add_argument(
        "--log-file",
        help="Write this run to a single file under Week5/logs"
    )
...
def main():
    ...
    logger.info(f"Starting {mode} pandas ETL run")
    """
    ...
    try:
        ...
        logger.info(f"{mode} pandas ETL run completed successfully")
    except Exception:
        logger.exception(f"{mode} pandas ETL run failed")
        raise
    finally:
        if src_conn:
            src_conn.close()
