...
def transform_trips(trips_df: pd.DataFrame, lookups: dict) -> pd.DataFrame:
    ...
    bad_vehicle = df["vehicle_id"].notna() & df["vehicle_key"].isna()
    if bad_vehicle.any():
        logger.warning(f"{bad_vehicle.sum()} trip(s) with unknown vehicle_id — skipped")
    df = df[~bad_vehicle]
    ...
    df["time_key"] = df["requested_at"].dt.hour * 100 + (df["requested_at"].dt.minute // 15) * 15
    ...
    for col in ("driver_key", "passenger_key", "vehicle_key", "pickup_location_key",
