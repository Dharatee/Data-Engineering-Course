...
SOURCE_DB_CONFIG = dict(
    host=    os.getenv("SRC_DB_HOST", os.getenv("DB_HOST")),
    port =   os.getenv("SRC_DB_PORT", os.getenv("DB_PORT")),
    dbname = os.getenv("SRC_DB_NAME", os.getenv("DB_NAME")),
    user=    os.getenv("SRC_DB_USER", os.getenv("DB_USER")),
...
def extract(conn, sql, params=None): #params is for watermark, which is optional
    try:
...
# define extract_vehicle function to extract vehicle data from the source database
...
# define load_dim_vehicle function to load vehicle data into the destination database
...
# Get watermark for fact_trips table to extract only new trips from the source database
...
def load_fact_trips(conn, fact_data):
 ...
 INSERT INTO fact_trips
    (source_trip_id, date_key, driver_key, passenger_key, vehicle_key,
...
def main():
    ...
    try:
        ...
        watermark = get_fact_watermark(dst_conn)
